...
int nb_waypoints_fini = 0;
Etat etat_actuel = RECEPTION_WAYPOINTS;
bool f_signal_before = false;
...
void init_trajectoire(Trajectoire &trajectoire, Point_marvelmind marvel_pos, Robot &robot){
  ...
  Serial.println("t0 : " + String(trajectoire.t0)); 
  Serial.println("tf : " + String(trajectoire.tf)); 
...
void readwaypoints(struct Trajectoire &trajectoire, bool f_signal_before) {
    char buffer_waypoints[200];
...
void loop() {
  ...
  switch (etat_actuel) {
    ...
    case ENVOI_FIN:
      Serial.println("ENVOI_FIN"); 
      Serial2.write("f\n"); 
      etat_actuel = RECEPTION_WAYPOINTS;
      f_signal_before = true;
      break;
    
    case RECEPTION_WAYPOINTS:
      Serial.println("RECEPTION_WAYPOINTS");
      readwaypoints(trajectoire, f_signal_before);
      Serial.println("x du dernier waypoint : " + String(trajectoire.waypoints[trajectoire.nb_waypoints-1].x)); 
      etat_actuel = INITIALISATION_TRAJ;
      break;

    case INITIALISATION_TRAJ:
      Serial.println("INITIALISATION_TRAJ"); 
      init_trajectoire(trajectoire, marvel_pos, robot); 
    ...
    case EXECUTION_TRAJ:
      ...
      Serial.println("Waypoints " + String(nb_waypoints_fini)+ " fait");
      if (nb_waypoints_fini == trajectoire.nb_waypoints){
        parcours_fini = true;
      }

      long t = millis();
      Serial.println("parcours fini : " + String(parcours_fini));
      if (parcours_fini){
        Serial.println("fini de faire la trajectoire"); 
